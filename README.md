...
### テストの実行方法
LaravelアプリケーションのPHPUnitテストは、以下の手順で実行できます。
1.  PHPコンテナに入ります。
    ```bash
...
2.  PHPUnitテストを実行します。
    * すべてのテストを実行する場合:
        ...
        php artisan test
        ```
    * 特定の機能テストのみを実行する場合 (例: `Feature` ディレクトリ内のテスト):
        ```bash
        php artisan test tests/Feature
        ```
    * 特定のテストクラスのみを実行する場合 (例: `RegisterTest` クラス):
        ```bash
...
* **Stripe決済処理の挙動（コンビニ決済におけるテスト環境の制約と合意事項）:**
    ...
    * この制約を踏まえ、コンビニ決済を選択した場合は、Stripeの決済完了画面まで遷移した後、**手動で `http://localhost/item/:item_id/purchased` (`:item_id` は実際の商品ID) のURLにアクセスすることで購入処理が完了する**よう実装しています。
